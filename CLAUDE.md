...
- All agents pin **sonnet** in frontmatter (cost efficiency) — do not change without good reason. The pin is the default, not a ceiling: a command may escalate an *individual spawn* via the Task call's `model` parameter (which overrides frontmatter for that spawn only) where a wrong answer is expensive — currently `/apple:review`'s Critical verifiers (Opus)
